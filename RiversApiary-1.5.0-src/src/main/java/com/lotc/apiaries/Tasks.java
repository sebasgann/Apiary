...
public final class Tasks {
    ...
    public static void startSchedulers(ApiaryPlugin plugin) {
        ...
        // --- Megachilidae Return Trail (FALLING_NECTAR + Crop Growth Pulse, Balanced) ---
        megachilidaeTrail = Bukkit.getScheduler().runTaskTimer(plugin, () -> {
            for (World w : Bukkit.getWorlds()) {
                for (Bee bee : w.getEntitiesByClass(Bee.class)) {
                    if (!bee.isValid() || !bee.hasNectar()) continue; // Only returning bees
                    ...
                    // Reduced FALLING_NECTAR trail — 1/4th density, 1-block radius
                    w.spawnParticle(Particle.FALLING_NECTAR, loc, 1, 0.15, 0.15, 0.15, 0.01);

                    // Every 10 ticks (0.5s), pulse crop growth in smaller 2-block radius below flight path
                    if (bee.getTicksLived() % 10 == 0) {
                        for (int dx = -2; dx <= 2; dx++) {
                            for (int dz = -2; dz <= 2; dz++) {
                                ...
                                if (Math.random() < 0.25) { // 25% growth chance (balanced)
                                    age.setAge(age.getAge() + 1);
                                    block.setBlockData(age);
                                    // Growth visual: SCRAPE particle for subtler feedback
        ...
        // --- Incubation Tick (Queen generation via Royal Jelly) ---
