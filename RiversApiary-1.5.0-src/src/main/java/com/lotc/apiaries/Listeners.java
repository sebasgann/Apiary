...
public class Listeners implements Listener {
    ...
    public Listeners(ApiaryPlugin plugin){ this.plugin = plugin; }
    ...
    public void onPlace(BlockPlaceEvent e){
        ItemStack item = e.getItemInHand(); if (item == null) return;
        ItemMeta im = item.getItemMeta(); if (im == null) return;
        var ip = im.getPersistentDataContainer();

        // Block unplaceable custom items (smoker / propolis), do NOT block apiary or wild hive
        Byte up = ip.get(Items.nk("unplaceable"), PersistentDataType.BYTE);
        if (up != null && up == 1) { e.setCancelled(true); return; }

        boolean managed = ip.has(Items.nk("managed"), PersistentDataType.STRING);
        boolean frontier = ip.has(Items.nk("frontier"), PersistentDataType.STRING);
        ...
        if (managed) p.set(HiveManager.KEY_MANAGED, PersistentDataType.STRING, "1");
        else p.set(HiveManager.KEY_FRONTIER, PersistentDataType.STRING, "1");

        // carry PDC if present on item
        copyIfPresent(ip, p, HiveManager.KEY_HONEY, PersistentDataType.INTEGER);
        copyIfPresent(ip, p, HiveManager.KEY_WAX, PersistentDataType.INTEGER);
        copyIfPresent(ip, p, HiveManager.KEY_CAP_H, PersistentDataType.INTEGER);
        copyIfPresent(ip, p, HiveManager.KEY_CAP_W, PersistentDataType.INTEGER);
        copyIfPresent(ip, p, HiveManager.KEY_TMAXBONUS, PersistentDataType.DOUBLE);
        copyIfPresent(ip, p, HiveManager.KEY_QUEEN, PersistentDataType.STRING);
        ts.update(true);

        // spawn starter bees (2)
        for (int i = 0; i < 2; i++){
    ...
    public void onBreak(BlockBreakEvent e){
        ...
        // save PDC into dropped Apiary item
        ...
        copyIfPresent(p, ipdc, HiveManager.KEY_HONEY, PersistentDataType.INTEGER);
        copyIfPresent(p, ipdc, HiveManager.KEY_WAX, PersistentDataType.INTEGER);
        copyIfPresent(p, ipdc, HiveManager.KEY_CAP_H, PersistentDataType.INTEGER);
        copyIfPresent(p, ipdc, HiveManager.KEY_CAP_W, PersistentDataType.INTEGER);
        copyIfPresent(p, ipdc, HiveManager.KEY_TMAXBONUS, PersistentDataType.DOUBLE);
        copyIfPresent(p, ipdc, HiveManager.KEY_QUEEN, PersistentDataType.STRING);
        drop.setItemMeta(im);
